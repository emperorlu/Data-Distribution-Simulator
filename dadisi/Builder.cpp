...
void crush_finalize(struct crush_map *map)
{
	...
	/* allocate arrays */
	map->device_parents = (uint32_t*)malloc(sizeof(map->device_parents[0]) * map->max_devices);
	memset(map->device_parents, 0, sizeof(map->device_parents[0]) * map->max_devices);
	map->bucket_parents = (uint32_t*)malloc(sizeof(map->bucket_parents[0]) * map->max_buckets);
...
int crush_add_rule(struct crush_map *map, struct crush_rule *rule, int ruleno)
{
	...
	if (ruleno >= map->max_rules) {
		...
		map->rules = (crush_rule**)realloc(map->rules, map->max_rules * sizeof(map->rules[0]));
...
struct crush_rule *crush_make_rule(int len, int ruleset, int type, int minsize, int maxsize)
{
	...
	rule = (crush_rule *)malloc(crush_rule_size(len));
	rule->len = len;
		     ...
		     struct crush_bucket *bucket)
{
	...
	while (pos >= map->max_buckets) {
		...
		if (map->max_buckets)
			...
			map->max_buckets = 8;
		map->buckets = (crush_bucket**)realloc(map->buckets, map->max_buckets * sizeof(map->buckets[0]));
			  ...
			  int item_weight)
{
	...
	bucket = (struct crush_bucket_uniform *)malloc(sizeof(*bucket));
	...
	bucket->h.items = (int32_t*)malloc(sizeof(int32_t)*size);
	bucket->h.perm = (uint32_t*)malloc(sizeof(uint32_t)*size);
	for (i=0; i<size; i++)
		       ...
		       int *weights)
{
	...
	bucket = (crush_bucket_list *) malloc(sizeof(*bucket));
	memset(bucket, 0, sizeof(*bucket));
	...
	bucket->h.items = (int32_t*)malloc(sizeof(int32_t)*size);
	bucket->h.perm = (uint32_t*)malloc(sizeof(uint32_t)*size);
	bucket->item_weights = (uint32_t*)malloc(sizeof(uint32_t)*size);
	bucket->sum_weights = (uint32_t*)malloc(sizeof(uint32_t)*size);
	w = 0;
	for (i=0; i<size; i++) {
		bucket->h.items[i] = items[i];
		       ...
		       int *weights)
{
	...
	bucket = (crush_bucket_tree *)malloc(sizeof(*bucket));
	memset(bucket, 0, sizeof(*bucket));
	...
	bucket->h.items = (int32_t*)malloc(sizeof(uint32_t)*size);
	bucket->h.perm = (uint32_t*)malloc(sizeof(uint32_t)*size);
	...
	bucket->node_weights = (uint32_t*)malloc(sizeof(uint32_t)*bucket->num_nodes);

	memset(bucket->h.items, 0, sizeof(uint32_t)*bucket->h.size);
			...
			int *weights)
{
	...
	bucket = (crush_bucket_straw *)malloc(sizeof(*bucket));
	memset(bucket, 0, sizeof(*bucket));
	...
	bucket->h.perm = (uint32_t*)malloc(sizeof(uint32_t)*size);
	bucket->item_weights = (uint32_t*)malloc(sizeof(uint32_t)*size);
	bucket->straws = (uint32_t*)malloc(sizeof(uint32_t)*size);
